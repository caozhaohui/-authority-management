    ...
    public PageResult queryAllRole(int pageNum, int pageSize) {
        ...
        for (int i = 0; i < list.size(); i++) {
            try {
                ...
                role.setCreateTime((Date) DateUtils.toDate(list.get(i).get("create_time").toString()));
    ...
    public Role queryRoleByName(String name) {
        ...
        if (list.size() > 0) {
            try {
                ...
                role.setCreateTime((Date) DateUtils.toDate(list.get(0).get("create_time").toString()));
